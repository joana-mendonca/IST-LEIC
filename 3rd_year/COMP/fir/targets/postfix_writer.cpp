...
void fir::postfix_writer::do_double_node(cdk::double_node * const node, int lvl) {
  ...
  _pf.SDOUBLE(node->value()); // output string characters
}
...
void fir::postfix_writer::do_read_node(fir::read_node * const node, int lvl) {
  ...
  _pf.STINT();  //write the value on the given address
}
...
//---------------------------------------------------------------------------
void fir::postfix_writer::do_pointer_index_node(fir::pointer_index_node *const node, int lvl) {
  //TO-DO
}
