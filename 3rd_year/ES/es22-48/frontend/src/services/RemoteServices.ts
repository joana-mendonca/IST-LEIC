...
export default class RemoteServices {
  ...
  static async getWeeklyScores(id: number): Promise<WeeklyScores[]> {
      return httpClient
        .get(
            `/students/dashboards/${id}/weeklyscores`
        )
        .then((response) => {
            return response.data.map((weeklyScore: any) => {
                return new WeeklyScores(weeklyScore);
            });
        })
        .catch(async (error) => {
            throw Error(await this.errorMessage(error));
    ...
    static async updateWeeklyScore(id: number) {
        return httpClient.put(
            `/students/dashboards/${id}/weeklyscores`
        );
    }

  static async deleteWeeklyScore(weeklyScoreId: number | undefined) {
    return httpClient
  ...
  static async deleteFailedAnswer(dashboardId: number, failedAnswerId: number) {
    return httpClient
      .delete(
        `/students/dashboards/${dashboardId}/failedAnswers/${failedAnswerId}`
      )
      .catch(async (error) => {
        throw Error(await this.errorMessage(error));
      });
  ...
  static async updateFailedAnswers(dashboardId: number) {
    return httpClient
      .put(
        `/students/dashboards/${dashboardId}`
      )
  }
