...
public class Dashboard implements DomainEntity {
    ...
    @OneToMany(cascade = CascadeType.ALL, mappedBy = "dashboard", orphanRemoval = true)
    private Set<FailedAnswer> failedAnswers = new HashSet<FailedAnswer>();
    ...
    public Dashboard(CourseExecution courseExecution, Student student) {
        ...
        //TemporalAdjuster weekSunday = TemporalAdjusters.previousOrSame(DayOfWeek.SUNDAY); //TODO (ERE)
        //LocalDate week = DateHandler.now().with(weekSunday).toLocalDate(); //TODO (ERE)
    }
